...
array.forEach((element) => {
  const filterNums = Array.from(element).filter((element) =>
    nums.includes(element)
  );
  if (filterNums.length === 1) {
    const onlyNum = filterNums[0];
    stringList.push(String(onlyNum) + String(onlyNum));
  } else if (filterNums.length > 1) {
    const firstNum = filterNums[0];
    const lastNum = filterNums[filterNums.length - 1];
    stringList.push(String(firstNum) + String(lastNum));
  }
});

const numList = stringList.map((element) => Number(element));
const result = numList.reduce((a, b) => a + b);
console.log(result);
