// Je li rezutat moguć ako ukupno imamo 12 crvenih, 13 zelenih i 14 plavih kockica? Ako je, ID tog game-a dodajemo u ukupni zbroj. Kao rješenje zadatka tražimo zbroj Id-jeva koji su mogući rezultati.
...
const input = `Game 1: 13 green, 3 red; 4 red, 9 green, 4 blue; 9 green, 10 red, 2 blue
...
parsedInput.forEach((game) => {
  const ID = game.slice(5, game.indexOf(":"));
  const rounds = game.slice(game.indexOf(":") + 2).split(";");
  let singleRoundBlues = 0;
  let singleRoundReds = 0;
  let singleRoundGreens = 0;
  rounds.forEach((round) => {
    const singleRound = round.split(",");
    singleRound.forEach((singleRound) => {
      if (singleRound.includes("blue")) {
        const blue = singleRound.slice(0, singleRound.indexOf("b")).trim();
        if (singleRoundBlues === 0) {
          singleRoundBlues = Number(blue);
        } else if (Number(blue) > singleRoundBlues) {
          singleRoundBlues = Number(blue);
        }
      }
      if (singleRound.includes("green")) {
        const green = singleRound.slice(0, singleRound.indexOf("g")).trim();
        if (singleRoundGreens === 0) {
          singleRoundGreens = Number(green);
        } else if (Number(green) > singleRoundGreens) {
          singleRoundGreens = Number(green);
        }
      }
      if (singleRound.includes("red")) {
        const red = singleRound.slice(0, singleRound.indexOf("r")).trim();
        if (singleRoundReds === 0) {
          singleRoundReds = Number(red);
        }
        if (Number(red) > singleRoundReds) {
          singleRoundReds = Number(red);
        }
      }
    });
  });
  possibleOutcomes.push(singleRoundBlues * singleRoundGreens * singleRoundReds);
});
console.log(possibleOutcomes.reduce((a, b) => a + b, 0));
